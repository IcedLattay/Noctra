...
/**
 * GenericTimerActivityFragment — Shape B: Stepper.
 *
 ...
 *
 * Step data (name/instruction/durations per sub-step) isn't part of the DB
 * schema, so it's looked up locally by activity label — see STEPS_BY_LABEL
 * below. Title/instruction for the pre-countdown panel still come from
 * routineViewModel.currentActivity, consistent with the rest of the app.
 *
 * IMPORTANT: this fragment computes its own total duration (sum of all step
 * action+rest durations) and passes that into
 * routineViewModel.startCurrentActivityTimer(), rather than using
 * Activity.defaultDurationMinutes from the DB — those DB values (8 min for
 * PMR, 7 min for Stretching) don't match the actual step-level timing
 * (75s / 90s respectively). If you want the DB column to be the source of
 * truth instead, update it to match, then switch this fragment to use it.
 */
class GenericTimerActivityFragment : Fragment() {
    ...
    private fun showStepPanel() {
        ...
        val totalDurationSeconds = steps.sumOf { it.actionDurationSeconds + it.restDurationSeconds }
    ...
    private fun runStep(index: Int, isAction: Boolean) {
        ...
        if (isAction) {
            ...
            runStepTimer(step.restDurationSeconds, isAction = false) {
                ...
                // else: all steps done, hold on this rest screen until VM's
                // own countdown (started with the same total duration) fires
                // GoToTransition — see class doc above.
            }
        }
    }
